...
class WindowAttention(nn.Module):
    ...
    def forward(self, feats, xyz, index_0, index_1, index_0_offsets, n_max):
        ...
        print("DEBUG: relative_position_index min:", relative_position_index.min().item(), "max:", relative_position_index.max().item(), "shape:", relative_position_index.shape)    
...
class BasicLayer(nn.Module):
    ...
    def forward(self, feats, xyz, offset):
        # feats: N, C
        # xyz: N, 3
        print(f"[DEBUG] xyz.shape: {xyz.shape}, offset: {offset}, sum(offset): {offset[-1].item() if len(offset) > 0 else 0}")
        window_size = torch.tensor([self.window_size]*3).type_as(xyz).to(xyz.device)
        
        batch = torch.zeros(xyz.shape[0], dtype=torch.long, device=xyz.device)
        start = 0
