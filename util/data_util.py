...
def collate_fn_limit_mix3d(batch, max_batch_points, logger, p):
    ...
    out_coord = torch.cat(coord[:k])
    out_feat = torch.cat(feat[:k])
    out_label = torch.cat(label[:k])
    out_offset = torch.IntTensor(offset[:k])

    # 检查 offset[-1] 是否等于 out_coord.shape[0]
    print(f"[DEBUG] collate_fn_limit: out_coord.shape[0]={out_coord.shape[0]}, out_offset[-1]={out_offset[-1].item() if len(out_offset) > 0 else None}")
    assert len(out_offset) == 0 or out_offset[-1].item() == out_coord.shape[0], \
        f"offset[-1]={out_offset[-1].item()}, coord.shape[0]={out_coord.shape[0]}"
...
def collate_fn_limit(batch, max_batch_points, logger):
    ...
    k = 0
    for item in coord:
        if count + item.shape[0] > max_batch_points:
            break
        count += item.shape[0]
        k += 1
        offset.append(count)
    ...
    out_feat = torch.cat(feat[:k])
    out_label = torch.cat(label[:k])
    out_offset = torch.IntTensor(offset)
    # 检查
    print(f"[DEBUG] collate_fn_limit: out_coord.shape[0]={out_coord.shape[0]}, out_offset[-1]={out_offset[-1].item() if len(out_offset) > 0 else None}")
    assert len(out_offset) == 0 or out_offset[-1].item() == out_coord.shape[0], \
        f"offset[-1]={out_offset[-1].item()}, coord.shape[0]={out_coord.shape[0]}"
